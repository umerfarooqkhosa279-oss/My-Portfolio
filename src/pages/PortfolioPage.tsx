...
<p  className="mt-6 max-w-2xl text-lg leading-8 text-slate-600 dark:text-slate-300 text-justify" style={{ textAlign: 'justify' }}> {home.hero_text} </p>
          ...
          <div className="container-app mt-8 grid gap-5 lg:grid-cols-[1fr_260px]">
            {!!home.achievements?.length && (
              <Card>
                ...
                <div className="grid gap-3 md:grid-cols-3">{home.achievements.map((a: string) => <div key={a} className="rounded-2xl bg-slate-50 p-4 text-sm dark:bg-slate-900" style={{ textAlign: 'justify' }} ><Star className="mb-3 text-brand-accent" size={18} />{a}</div>)}</div>
              </Card>
            )}
